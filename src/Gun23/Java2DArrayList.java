...
public class Java2DArrayList {
    public static void main(String[] args) {
        ...
        System.out.println("tümdersort(notlarListesi) = " + tümdersort(notlarListesi));

        // Soru 4 : En büyük ve en küçük notu bir maın ıcınde bulunuz (fonksiyon yok)

        enbenk(notlarListesi);


    }
    public static void enbenk(ArrayList<ArrayList<Integer>> notlarlistesi){

        for (int i = 0; i < notlarlistesi.size(); i++) {
            for (int j = 0; j < notlarlistesi.get(i).size(); j++) {
                System.out.println(Collections.max(notlarlistesi.get(i)));
                break;
            }
        }

    }
    public static double tümdersort(ArrayList<ArrayList<Integer>> notlarlistesi){
        double toplam=0;
        int k=0;
        for (int i = 0; i < notlarlistesi.size(); i++) {
            for (int j = 0; j < notlarlistesi.get(i).size(); j++) {
